...
SUBROUTINE PRINCIPAL_SIM_AUTO(ESCREVE_CABECALHO,K1RESET,K2RESET,K3RESET,FONTE,COMPDOMX,NIINIC,COMPDOMY,MJINIC,NB,&
...
IMPLICIT NONE 
 ...
 REAL(KIND=10),INTENT(IN)                       ::K1RESET,K2RESET,K3RESET,VELOCIDADE,W_SOL,AUTO_G_STEP,AUTO_G_MAX
 ...
 TYPE(F_GAMMA),DIMENSION(NB),INTENT(INOUT)      ::DADOS_GAMMA_X,DADOS_GAMMA_Y
 ...
 CHARACTER(20)                        ::NOME_PASTA
 CHARACTER(6)                         ::NOMEARQ_N
 LOGICAL                              ::CONVERGE,NEG_ENCONTRADO
 REAL(KIND=10)                        ::K1FINAL,K1PASSO
 

 K1FINAL=FONTE(1)%K1END
 K1PASSO=FONTE(1)%K1STEP
 ...
 !!! LOOPS AUTOMATICOS !!!
 !!!!!!!!!!!!!!!!!!!!!!!!!
 DO WHILE (DADOS_GAMMA_X(1)%GAMMA_ZERO < AUTO_G_MAX + 0.0001)
 
 DO WHILE (K1VAR < K1FINAL+0.0001)
 ...
 PRINT*,'TEMPO ATUAL:',T,'K1=',K1VAR
 PRINT*,'CICLO ATUAL:',CICLO,'FRAME:',FRAME,'G=',DADOS_GAMMA_X(1)%GAMMA_ZERO
 ...
 !WRITE(NOME_PASTA,'(I0.6)') AUTO_CONT                                               !NOMEIA PASTA
 IF (K1VAR < 10.0) THEN
    IF (DADOS_GAMMA_X(1)%GAMMA_ZERO < 10.0) THEN
        WRITE(NOME_PASTA,'("G1-",F4.2,"K1-",F4.2)') DADOS_GAMMA_X(1)%GAMMA_ZERO, K1VAR
    ELSE
        WRITE(NOME_PASTA,'("G1-",F5.2,"K1-",F4.2)') DADOS_GAMMA_X(1)%GAMMA_ZERO, K1VAR
    END IF
 ELSE
    IF (DADOS_GAMMA_X(1)%GAMMA_ZERO < 10.0) THEN
        WRITE(NOME_PASTA,'("G1-",F4.2,"K1-",F5.2)') DADOS_GAMMA_X(1)%GAMMA_ZERO, K1VAR
    ELSE
        WRITE(NOME_PASTA,'("G1-",F5.2,"K1-",F5.2)') DADOS_GAMMA_X(1)%GAMMA_ZERO, K1VAR
    END IF
 END IF
 
 MANIPULA = SYSTEM('mkdir SAI_AUTO/'//NOME_PASTA)                                    !CRIA PASTA
 ...
 DEALLOCATE (X,Y,XS,YS)
 DEALLOCATE(S2D,S2D_PAS,S2D_PAS_PAS)
 DEALLOCATE(S2D_LAMBDA,S2D_VEL_X,S2D_VEL_Y,S2D_VS)
 
 END DO !K1
 
 DADOS_GAMMA_X(1)%GAMMA_ZERO=DADOS_GAMMA_X(1)%GAMMA_ZERO+AUTO_G_STEP
 DADOS_GAMMA_Y(1)%GAMMA_ZERO=DADOS_GAMMA_Y(1)%GAMMA_ZERO+AUTO_G_STEP
 ...
 END SUBROUTINE PRINCIPAL_SIM_AUTO
